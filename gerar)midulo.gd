...
func _run() -> void:
	...
	for filho in instancia_dae.get_children():
		...
		# O 'filho' pode ser MeshInstance3D diretamente ou conter um
		if filho is MeshInstance3D:
			var malha : MeshInstance3D = filho.duplicate()
			malha.name          = "Malha"
			malha.position      = Vector3.ZERO  # reseta posição local
			malha.rotation      = Vector3.ZERO  # reseta rotação local
			raiz.add_child(malha)
			malha.owner = raiz
		else:
			# Procura o MeshInstance3D dentro do filho
			var malha_filho := _encontrar_mesh(filho)
			if malha_filho:
				var malha : MeshInstance3D = malha_filho.duplicate()
				malha.name     = "Malha"
				malha.position = Vector3.ZERO
				malha.rotation = Vector3.ZERO
				raiz.add_child(malha)
				malha.owner = raiz
			else:
